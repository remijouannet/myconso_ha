...
Provides index for every counter of your housings behind the [Myconso application](https://play.google.com/store/apps/details?id=fr.proxiserve.myconso&hl=fr)
...
docker run -i --name ha -v $(pwd)/config:/config -v $(pwd)/custom_components:/config/custom_components -e TZ=MY_TIME_ZONE -p 8123:8123 homeassistant/home-assistant:stable
docker rm ha
```
